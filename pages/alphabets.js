...
export default function Alphabets({ kalasha, error }) {
  ...
  const { alphabetHeading, letter, latin, nameOfLetter, usage, phoneme, rotate } = phrases
      ...
      {error ?
        (<SomethingWentWrong />) : (
          loadingPage ? <LoadingPage /> : (
            <div dir={dir} className="container my-10 flex flex-col justify-center bg-white p-5 rounded-xl max-w-[1000px] md:max-w-[700px] mx-2 sm:mt-[20px]">
              <div className='flex  flex-col justify-center items-center text-3xl my-4'>
                <h3 className="text-5xl  text-center sm:text-center mt-6 w-full"> {alphabetHeading[lan]} </h3>

              </div>
              <div className="md:hidden xl:hidden lg:hidden sm:flex flex-col">
              <p className='mt-5 w-full text-center text-2xl'>{rotate[lan]}</p>
              <Image src={rotateImage} alt="rotate your phone " width={100} height={200} />
              </div>
              <table dir={dir} className="border-2 text-xl sm:hidden">
                <thead className="border-2 text-center">
                  <tr>
                    ...
                    <th className={cells}> {nameOfLetter[lan]}
                    </th>
                    <th className={cells}>
                      {usage[lan]}
                    </th>
